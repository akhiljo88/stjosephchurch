...
import { Users, Phone, MapPin, User, Search, X } from 'lucide-react';
import Header from '../components/Header';
import Navigation from '../components/Navigation';
import HomeButton from '../components/HomeButton';
import Copyright from '../components/Copyright';
import { getFamilies } from '../lib/database';
...
const FamiliesDirectory: React.FC = () => {
        ...
        <div className="container mx-auto max-w-7xl">
          ...
          >
            ...
            <p className="text-gray-700 text-xl mt-8 font-serif max-w-3xl mx-auto">
              Meet the wonderful families that make up our church community. Each family brings their unique story and contributes to our shared faith journey.
            </p>
            ...
            >
              ...
              <p className="text-amber-600 text-sm font-serif text-center">- Joshua 24:15</p>
            </motion.div>
          </motion.div>
            ...
            {searchTerm && (
              <p className="mt-2 text-sm text-amber-600 font-serif text-center">
                {filteredFamilies.length === 0 
                  ...
                  : `Found ${filteredFamilies.length} famil${filteredFamilies.length === 1 ? 'y' : 'ies'} matching "${searchTerm}"`
                }
              </p>
            )}
          </motion.div>
          ...
          >
            {loading ? (
              <div className="text-center py-8">
                <p className="text-red-900 font-serif text-xl">Loading families...</p>
              </div>
            ) : filteredFamilies.length === 0 ? (
              <div className="text-center py-8">
                <p className="text-gray-500 font-serif text-xl">
                  {searchTerm ? `No families found matching "${searchTerm}"` : 'No families registered yet.'}
                </p>
              </div>
            ) : (
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                {filteredFamilies.map((family, index) => (
                  ...
                  >
                    ...
                    {/* Family Details */}
                    <div className="p-6">
                      <h3 className="text-xl font-bold text-red-900 mb-3 font-serif text-center">
                      ...
                      <div className="space-y-2 mb-4">
                        <div className="flex items-center">
                          <Phone className="w-4 h-4 text-amber-600 mr-2 flex-shrink-0" />
                          <button
                            onClick={() => handleCall(family.contactNumber)}
                            className="text-blue-600 hover:text-blue-800 transition-colors duration-200 font-serif text-sm"
                          >
                            {family.contactNumber}
                          </button>
                        </div>
                        <div className="flex items-start">
                          <MapPin className="w-4 h-4 text-amber-600 mr-2 flex-shrink-0 mt-0.5" />
                          <span className="text-gray-700 font-serif text-sm">{family.address}</span>
                        </div>
                      </div>

                      {/* Family Members */}
                      <div>
                        <h4 className="font-semibold text-red-900 mb-2 font-serif text-sm">Family Members:</h4>
                        <div className="space-y-1 max-h-32 overflow-y-auto">
                          {family.members.map((member, memberIndex) => (
                            <div key={memberIndex} className="flex items-center justify-between bg-amber-100 px-3 py-1 rounded-lg">
                              <div className="flex items-center">
                                <User className="w-3 h-3 text-amber-600 mr-2" />
                                <span className="text-red-900 font-serif text-sm font-medium">
                                  {member.name}
                                </span>
                              </div>
